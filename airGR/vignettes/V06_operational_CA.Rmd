...
source("/Volumes/Transcend/hydrosolutions Dropbox/Adrian Kreiner/SAPPHIRE_Central_Asia_Technical_Work/code/Al_Archa_model/05_Model/operational/04_Function/functions_operational.R")
...
Basin_lat <- basin_coords[, "Y"]
Basin_lat_rad <- Basin_lat * pi / 180

Basin_Info <- list(
  BasinCode = Basin_code,
  BasinName = river_name,
  BasinArea_m2 = BasinArea_m2,
  BasinLat_rad = Basin_lat_rad,
...
# filter defect years 
defect_years <- c("2016", "2002")
nc_files <- nc_files[!grepl(paste0(defect_years, collapse = "|"), nc_files)]
...
extend_and_smooth_data <- function(data, year = 2020, window_size = 30) {
  ...
  # Smooth the data
  data_extended$grad_Tmean_smooth <- zoo::rollmean(data_extended$grad_Tmean, window_size, fill = NA, align = 'center')
  ...
  data_final <- select(data_final, day, month, grad_Tmean_smooth) %>%
    rename(grad_Tmean = grad_Tmean_smooth) %>%
    mutate(grad_Tmean = round(grad_Tmean, 3)) %>%  # round to 3 digits
    mutate(grad_Tmin = 0, grad_Tmax = 0)  # Reset Min and Max as placeholders ( do not need for GR4J, GR6J model in the current implementation)
  return(data_final)
}

daily_stats_smoothed <- extend_and_smooth_data(daily_stats)
# write.table(daily_stats_smoothed, file = paste0(data_dir,"/LapsrateT_daily_1990_2019.txt"), row.names = FALSE, quote = FALSE, sep = ",", col.names = TRUE)
...
ggplot() +
  geom_line(data = daily_stats_smoothing_plot, aes(x = Date, y = grad_Tmean, colour = "Laps rate T"), color = "darkred", linewidth = 1.5) +
...
## 2.3  Visuale X-Check of the temperature lapse rates 
```{r}
# ========================
# Plotting the result 
# Create a data frame from the list
years <- seq(1990, length(lapsrate_t_years) + 1999)
data <- do.call(rbind, lapply(seq_along(lapsrate_t_years), function(i) {
  year_data <- data.frame(
    day = seq_along(lapsrate_t_years[[i]]),
    LapseRate = lapsrate_t_years[[i]],
    Year = years[i]
  )
  return(year_data)
}))
...
data <- data %>%
  filter(!Year %in% c(2016, 2002))

# Plotting all years with lapse rates over days
ggplot(data, aes(x = day, y = LapseRate, color = as.factor(Year), group = Year)) +
  geom_line() +  # Draw lines
  labs(title = "Daily T Lapse Rate by Year",
       x = "Day of Year [-]",
       y = "Lapse Rate T [°C/1000m]",
       color = "Year") +
  theme_minimal() +
  scale_color_viridis_d() +  # Use a discrete viridis color scale
  theme(axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14),
        axis.text.x = element_text(size = 12),  # Increase x-axis tick label size and make it bold
        axis.text.y = element_text(size = 12),
        title = element_text(size = 16)) + 
  guides(color = guide_legend(title = "Year",
                              ncol = 2,  # Set the number of columns in the legend to two
...
P_Z0 <- P_elev_data_4000 %>%
  filter(between(elevation, Z0 - 50, Z0 + 50))
P0 <- mean(P_Z0$value)
...
save(k_value, file = "/Volumes/Transcend/hydrosolutions Dropbox/Adrian Kreiner/SAPPHIRE_Central_Asia_Technical_Work/code/Al_Archa_model/05_Model/operational/01_BasinInfo/16936/k_value.RData")

# Visualization 
elevation_seq <- seq(min(lapsrate_p_data$elevation),Zthreshold, length.out = 100)
...
new_row <- data.frame(Elevation = 5000, Predicted_Value = value_at_max_elevation)
...
GradT <- read.table(file = paste0(data_dir,"/LapsrateT_daily_1990_2019.txt"), header = TRUE, sep = ",")
...
# 1. get the forecasts dates
# needed because want also to have the last date 
end_date_temp <- end_date + 1
forecast_dates <- pentadal_days(start_date, end_date)
...
for (i in 1:(length(forecast_dates)-1)) {
  forecast_date <- forecast_dates[i]
  print(paste0("Forecast date: ", forecast_date))
  ...
  Qsim_end <- tibble(date = as.Date(runResults_OL$DatesR[length(runResults_OL$Qsim)]),
                     Qsim_end = runResults_OL$Qsim[length(runResults_OL$Qsim)]) %>%
    left_join(basinObsTS_SWE %>% dplyr::select(date, Qmm), by = "date") %>%
    mutate(diff = Qsim_end - Qmm)

  
  

    runOptions_cheating <- airGR::CreateRunOptions(FUN_MOD = RunModel_CemaNeigeGR4J_Glacier,
                                    InputsModel = inputsModel,
                                    IndPeriod_Run = indRun_DA,
                                    IniStates = initstate, 
                                    IniResLevels     = NULL,
                                    IndPeriod_WarmUp = 0L,
                                    MeanAnSolidPrecip = MeanAnSolidPrecip,
  ...
  runResults_cheating <- RunModel_CemaNeigeGR4J_Glacier(InputsModel   = inputsModel,
                                                        RunOptions  = runOptions_cheating,
                                                        Param       = param)
  
  runResults_cheating_df <- tibble(date = as.Date(runResults_cheating$DatesR),
                                         Qsim = runResults_cheating$Qsim,
                                   Qsim_corrected = runResults_cheating$Qsim - Qsim_end$diff)
  
  result_cheating <- runResults_cheating_df %>%
    summarise(Qsim_corrected = mean(Qsim_corrected), 
              Qsim = mean(Qsim)) %>%
...
final_results_df$flag <- ifelse((final_results_df$Qsim_corrected<= (final_results_df$Qobs_pentad + final_results_df$factor)) & (final_results_df$Qsim_corrected >= (final_results_df$Qobs_pentad - final_results_df$factor)), 1, 0)
...
FA <- final_results_df %>%
  ...
  summarize(count_flag_1 = sum(flag == 1, na.rm = TRUE), # Count of flag=1 for each pentad
            ...
            percentage_flag_1 = (count_flag_1 / total_entries)) 


ggplot() + 
  geom_line(data = FA, aes(x = pentad, y = percentage_flag_1))+
  geom_line(data = FA_linreg, aes(x = pentad, y = percentage_flag_1))+
  geom_line(data = FA_model, aes(x = pentad, y = percentage_flag_1))
  

combined_data <- tibble(pentad_dates = FA_linreg$pentad_dates, 
                        pentad = FA_linreg$pentad,
                        linreg = FA_linreg$percentage_flag_1,
                        model = FA_model$percentage_flag_1,
                        Fake = FA$percentage_flag_1)


start_april <- as.Date("2016-04-01")
start_oct <- as.Date("2016-10-01")
ggplot() +

  
  
  geom_line(data = combined_data, aes(x = pentad_dates, y = linreg, color = "Lin. reg. latest version"), linewidth = 1) +
  geom_line(data = combined_data, aes(x = pentad_dates, y = Fake, color = "FAKE"),linewidth = 1) +
  geom_line(data = combined_data, aes(x = pentad_dates, y = model, color = "Model without DA"), linewidth = 1) +
  
  scale_color_manual(values = c("Model without DA" = "#52A073", "FAKE" = "#FF5733", "Lin. reg. latest version" = "black")) +
  geom_vline(xintercept = as.numeric(start_april), linetype = "dashed", color = "black")+
  geom_vline(xintercept = as.numeric(start_oct), linetype = "dashed", color = "black") +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.2)) +
  scale_x_date(date_labels = "%b %d", date_breaks = "1 months") +
   labs( title = "Ala Archa",
    x = 'Date', 
       y = 'Forecast Accuracy [-]', 
       color = "Legend", 
       fill = "Legend")+
