ErrorCrit_NSE_weighted <- function(InputsCrit, OutputsModel, warnings = TRUE, verbose = TRUE) {
  ...
  # print if there is OutputsModel$DatesR then print the first date
  if (!is.null(OutputsModel$DatesR)) {
      message(sprintf("First date: %s", OutputsModel$DatesR[1]))
  }
